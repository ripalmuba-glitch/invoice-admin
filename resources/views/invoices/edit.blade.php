    ...
    <x-slot name="header">
        <div class="bg-white shadow-md rounded-xl p-8">
            <h2 class="text-3xl font-bold text-gray-800 leading-tight">
                Edit Invoice: <span class="text-primary-600">{{ $invoice->invoice_number }}</span>
            </h2>
            <p class="text-md text-gray-600 mt-2">
                Perbarui detail invoice, item akan terhitung ulang secara otomatis.
            </p>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('invoices.update', $invoice) }}" method="POST">
                ...
                <div x-data="invoiceForm({
                        ...
                        initialItems: {{ old('items') ? json_encode(old('items')) : $invoice->items->toJson() }},
                        initialDiscount: {{ old('discount', $invoice->discount) }},
                        initialTaxPercent: {{ old('tax_percent', $invoice->tax_percent) }}
                     })"
                     x-init="init()"
                     class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select id="template" name="template" required
                                            class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                        <option value="template-yellow-modern" {{ (old('template', $invoice->template) == 'template-yellow-modern') ? 'selected' : '' }}>Modern (Kuning)</option>
                                        <option value="template-black-gold" {{ (old('template', $invoice->template) == 'template-black-gold') ? 'selected' : '' }}>Elegan (Hitam Emas)</option>
                                        <option value="template-blue-simple" {{ (old('template', $invoice->template) == 'template-blue-simple') ? 'selected' : '' }}>Simpel (Biru)</option>
                                        <option value="template-blue-geometric" {{ (old('template', $invoice->template) == 'template-blue-geometric') ? 'selected' : '' }}>Geometris (Biru)</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex justify-end pt-4 space-x-4">
                        <a href="{{ route('invoices.index') }}" class="inline-flex items-center px-8 py-3 bg-gray-200 text-gray-700 font-semibold rounded-lg shadow hover:bg-gray-300 transition-all">
                            Batal
                        </a>
